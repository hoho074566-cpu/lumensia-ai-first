...
function currentRuntimeState(pc = {}, scene = {}, relationships = {}, continuityMemory = {}) {
  ...
  const lines = [
    ...
    'PC 생성창에 적힌 이세계 출신·미정 학과·기타 신분·신원 불명·희귀 장비 같은 비정상적인 요소는 이 세계에서 플레이하기 위한 시작 전제이지, 자동으로 장기 수사·격리·검증 서사의 주제가 아니다. 사용자가 그 정체 규명 자체를 플레이하려는 의도를 보이지 않았고 즉각적인 위해도 확인되지 않았다면, 필요한 최소 안전 확인 뒤 추가 행정·검사·보고는 한두 문장으로 압축하거나 뒤에서 처리하고 PC가 수업·훈련·식사·기숙사·친교·교내 이동 같은 아카데미 생활에 실제로 접근할 수 있게 장면을 전진시킨다. 학과나 신분이 미정이라는 이유만으로 생활 진입을 무기한 보류하지 않는다. 반대로 현재 장면에서 실제 범죄·공격·명백한 위협이 발생했다면 그에 필요한 대응은 유지한다.',
