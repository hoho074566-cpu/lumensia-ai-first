...
function boundedInteger(value, min, max, label, { nullable = false } = {}) {
  ...
  if (!Number.isInteger(number) || number < min || number > max) throw new Error(`${label}은 ${min}~${max} 범위의 정수여야 합니다.`);
  return number;
}
...
export function safePc(raw = {}) {
  ...
  const pc = {
    name: cleanText(raw.name, 80).trim(), age: Number(raw.age), gender: cleanText(raw.gender, 40), department: cleanText(raw.department, 80),
...
function rawWriterInstructions(instructions = '') {
  const withoutStructuredContract = String(instructions).replace(`\n\n${STRUCTURED_PROMPT_FRAGMENT}`, '').trim();
...
export function buildWriterRequestBody({ authoring, outputMode = 'structured' } = {}) {
  ...
  const body = {
    ...
    input: authoring?.input || '', reasoning: { effort: 'medium' }, max_output_tokens: 5600,
  };
  if (!rawMode) body.text = { format: { type: 'json_schema', name: 'lumensia_authoring_scene', strict: true, schema: OUTPUT_SCHEMA } };
  return body;
}
...
export function proseToTurn(text, fallbackScene = {}) {
  ...
  const blocks = normalized.split(/\n{2,}/).flatMap((block) => splitLongBlock(block)).filter(Boolean).slice(0, 48);
  if (!blocks.length) throw new Error('Writer가 표시 가능한 장면 본문을 반환하지 않았습니다.');
  return {
    scene: blocks.map((block) => ({ kind: 'narration', text: block, speaker_key: null, speaker_name: null, expression: null })),
    continuity: {
      date: fallbackScene.date, time: fallbackScene.time, location: fallbackScene.location, situation: fallbackScene.situation,
      present_character_keys: Array.isArray(fallbackScene.presentCharacterKeys) ? [...fallbackScene.presentCharacterKeys] : [],
    },
  };
}

export function validateTurn(turn, pc, fallbackScene) {
  if (!turn || typeof turn !== 'object' || !Array.isArray(turn.scene) || !turn.scene.length) throw new Error('Writer가 유효한 scene을 반환하지 않았습니다.');
  const scene = turn.scene.slice(0, 28).map((beat) => {
    ...
    if (kind === 'dialogue') {
      ...
      if (!registeredKey && !speakerName) return { kind: 'narration', text, speaker_key: null, speaker_name: null, expression: null };
      return { kind, text, speaker_key: registeredKey, speaker_name: registeredKey ? null : speakerName, expression: registeredKey && EXPRESSIONS.has(beat?.expression) ? beat.expression : (registeredKey ? 'default' : null) };
    }
    return { kind, text, speaker_key: null, speaker_name: null, expression: null };
  });
  if (pc.name !== 'Aaa' && scene.some((beat) => /\bAaa\b/.test(beat.text))) throw new Error('Writer가 PC 이름 대신 legacy placeholder Aaa를 사용했습니다.');
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const relationships = runState.relationships && typeof runState.relationships === 'object' && !Array.isArray(runState.relationships) ? runState.relationships : {};
    const continuityMemory = runState.continuityMemory && typeof runState.continuityMemory === 'object' && !Array.isArray(runState.continuityMemory) ? runState.continuityMemory : {};
    ...
    const authoring = assembleAuthoring({ action, pc, scene, relationships, continuityMemory, history, mode, inputKind, contextMode: writerContextMode });
    const apiResponse = await fetch('https://api.openai.com/v1/responses', {
      method: 'POST', headers: { Authorization: `Bearer ${process.env.OPENAI_API_KEY}`, 'Content-Type': 'application/json' },
      body: JSON.stringify(buildWriterRequestBody({ authoring, outputMode: writerOutputMode })), signal: AbortSignal.timeout(120_000),
    });
    const raw = await apiResponse.text();
    let response;
    try { response = raw ? JSON.parse(raw) : {}; }
    catch { throw new Error(`OpenAI가 JSON이 아닌 응답을 반환했습니다. HTTP ${apiResponse.status}`); }
    if (!apiResponse.ok) { const error = new Error(response?.error?.message || `OpenAI 요청 실패: HTTP ${apiResponse.status}`); error.status = apiResponse.status; throw error; }
    const outputText = extractOutputText(response);
    if (!outputText) throw new Error('Writer 응답 본문이 비어 있습니다.');
    if (pc.name !== 'Aaa' && /\bAaa\b/.test(outputText)) throw new Error('Writer가 PC 이름 대신 legacy placeholder Aaa를 사용했습니다.');
    let turn;
    if (writerOutputMode === 'raw') turn = proseToTurn(outputText, scene);
    else {
      let parsed;
    ...
    return json(res, 200, {
      turn, admin_preview: adminScenePreview, continue_scene: continueScene, writer_output_mode: writerOutputMode, writer_context_mode: writerContextMode,
      input_kind: inputKind, continuity_frozen_for_parity: writerOutputMode === 'raw', continuity_persisted_via_state_keeper: true,
      model: response?.model || process.env.OPENAI_MODEL || 'gpt-5.6-terra', request_id: response?.id || null, usage: response?.usage || null,
      academy_cast_count: ACADEMY_CAST_KEYS.length, authoring_diagnostics: authoring.diagnostics,
    });
  } catch (error) {
    const timeout = error?.name === 'TimeoutError' || error?.name === 'AbortError';
    const status = timeout ? 504 : (Number.isInteger(error?.status) ? error.status : 500);
    return json(res, status, { error: timeout ? 'Writer 응답 시간이 초과되었습니다. 게임 상태는 변경되지 않았습니다.' : (error?.message || 'Writer 요청 중 오류가 발생했습니다.'), code: timeout ? 'WRITER_TIMEOUT' : 'WRITER_ERROR' });
  }
}
