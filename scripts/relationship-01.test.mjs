...
const firstMeaningful = applyRelationshipDecision({
  ...
  decision: {
    ...
    relationship_changes: [{ npc_key: 'sera', before_main: '아는 사이', after_main: '호감', before_aux: [], after_aux: [], reason: '도움을 받고 좋은 인상을 받았다.', notice: '세라가 당신에게 호감을 느끼기 시작합니다.' }],
  },
...
const auxiliarySignal = applyRelationshipDecision({
  ...
  decision: {
    ...
    relationship_changes: [{ npc_key: 'isabel', before_main: '아는 사이', after_main: '아는 사이', before_aux: [], after_aux: ['흥미'], reason: '대련을 계기로 개인적인 관심이 생겼다.', notice: '이사벨이 당신에게 흥미를 보이기 시작합니다.' }],
  },
...
const accumulated = applyRelationshipDecision({
  relationships: {
    sera: { main: '아는 사이', aux: ['경계'], evidence: [{ id: 'old-rel', note: '여러 차례 훈련을 함께하며 약속을 지키는 모습을 봤다.', significance: 'meaningful', date: '1285-03-02', time: '15:00' }], changes: [], updatedAt: '1285-03-02 15:00' },
  },
  decision: {
    relationship_observations: [{ npc_key: 'sera', evidence: '위험한 순간 PC를 먼저 믿고 등을 맡겼으며 이후 먼저 말을 걸었다.', significance: 'meaningful' }],
    relationship_changes: [{ npc_key: 'sera', before_main: '아는 사이', after_main: '호감', before_aux: ['경계'], after_aux: ['신뢰'], reason: '누적된 신뢰와 이번 행동으로 감정의 의미가 달라졌다.', notice: '세라가 당신에게 호감을 느끼기 시작합니다.' }],
  },
...
const confessionOnly = applyRelationshipDecision({
  relationships: { sera: { main: '호감', aux: ['신뢰'], evidence: [{ id: 'old', note: '서로 호감을 확인해 가는 중이다.', significance: 'meaningful' }], changes: [] } },
  decision: {
    relationship_observations: [{ npc_key: 'sera', evidence: 'PC가 고백했지만 세라는 즉답하지 않고 생각할 시간을 달라고 했다.', significance: 'meaningful' }],
    relationship_changes: [{ npc_key: 'sera', before_main: '호감', after_main: '연인', before_aux: ['신뢰'], after_aux: ['신뢰'], reason: '고백이 있었다.', notice: '세라가 당신의 고백을 받아들였습니다.' }],
  },
});
...
const acceptedConfession = applyRelationshipDecision({
  ...
  decision: {
    ...
    relationship_changes: [{ npc_key: 'sera', before_main: '호감', after_main: '연인', before_aux: ['신뢰'], after_aux: ['신뢰'], reason: '상호 고백 수락과 연애 관계 성립이 장면에서 명확히 확정됐다.', notice: '세라가 당신의 고백을 받아들였습니다.' }],
  },
});
...
const unknownNpc = applyRelationshipDecision({
  relationships: {},
  decision: { relationship_observations: [{ npc_key: 'made_up_npc', evidence: '임의 인물', significance: 'milestone' }], relationship_changes: [] },
});
...
assert.equal((stateKeeper.match(/https:\/\/api\.openai\.com\/v1\/responses/g) || []).length, 1, 'growth, relationships and continuity must share one bookkeeping State Keeper call');
assert.equal((client.match(/fetch\('\/api\/state-keeper'/g) || []).length, 1, 'client must not add a third relationship model call');
assert.match(authoringRuntime, /currentRuntimeState\(pc, scene, relationships, continuityMemory\)/, 'Writer relationship knowledge must still enter through runtime facts');
