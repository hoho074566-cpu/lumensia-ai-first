...
assert.match(api, /assembleAuthoring/, 'Writer endpoint must use the Authoring Runtime assembly boundary');
...
assert.doesNotMatch(authoringRuntime, /buildCanonContext|relevantCharacterKeys|relevantScheduleFacts/, 'Crack-style Writer path must not use turn-level relevance/schedule routing');
assert.match(authoringRuntime, /characters\.json/, 'Crack-style Add-ons must be assembled from factual character Canon');
assert.match(authoringRuntime, /character-state\.json/, 'Crack-style Add-ons must include dated character state');
assert.match(authoringRuntime, /baseline\.json/, 'Crack-style scenario Add-on must use the dated scenario source');
assert.match(api, /maxItems:\s*28/, 'scene schema should remain a free ordered beat stream with ample room');
assert.match(canonContext, /ACADEMY_PRESENCE/, 'factual Canon tooling must still discover academy cast from dated presence');
assert.doesNotMatch(canonContext, /EVERYDAY_ACADEMY_CAST/, 'factual Canon tooling must not maintain a separate hand-picked everyday cast');
...
console.log('PASS V0 AI-first runtime invariants under Crack-style Authoring Runtime');
